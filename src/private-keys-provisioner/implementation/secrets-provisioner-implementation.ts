...
export const create = (deps: Dependencies) => (props: Props): API.SecretsProvisioner => {
  ...
  return {
    provision: async () => {
      ...
      const accessTokenKeyStillValid = latestAccessTokenVersionCreationDate && 
          now.minus(refreshInterval) <= luxon.DateTime.fromJSDate(latestAccessTokenVersionCreationDate);

      const refreshTokenKeyStillValid = latestRefreshTokenVersionCreationDate && 
        now.minus(refreshInterval) <= luxon.DateTime.fromJSDate(latestRefreshTokenVersionCreationDate);
